...
# Bootstrap a dev-only web/config/settings.php on first run. The file is
# gitignored, so a fresh clone has no settings; rather than hard-stop, we
# generate random secrets so the stack just works. Existing files are kept
# untouched.
if [[ ! -f web/config/settings.php ]]; then
  echo "==> generating web/config/settings.php (dev-only secrets)"
  ...
  cat > web/config/settings.php <<PHP
<?php
// Auto-generated by docker/smoke-test.sh for the local docker-compose stack.
// Safe to delete: the script will regenerate it (with new secrets) on the
// next run. Do NOT use these values in production.
return [
    'db.options' => [
        'dbname' => 'agendav',
        'user' => 'agendav',
    ...
    // The bind-mounted var/ directory is owned by the host user, so the
    // auto-generated var/session.key fallback cannot be created by Apache
    // (www-data). For real installs, omit this line and let AgenDAV manage
    // var/session.key itself.
    'session.encryption.key' => '${SESSION_KEY}',
    'log.level' => 'DEBUG',
