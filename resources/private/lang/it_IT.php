<?php
/*
 *  This file is part of AgenDAV.
 *
 *  AgenDAV is free software: you can redistribute it and/or modify
 *  it under the terms of the GNU General Public License as published by
 *  the Free Software Foundation, either version 3 of the License, or
 *  any later version.
 *
 *  AgenDAV is distributed in the hope that it will be useful,
 *  but WITHOUT ANY WARRANTY; without even the implied warranty of
 *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *  GNU General Public License for more details.
 *
 *  You should have received a copy of the GNU General Public License
 *  along with AgenDAV.  If not, see <http://www.gnu.org/licenses/>.
 */

$translations = array(
    'labels.username' => 'Nome Utente',
    'labels.password' => 'Password',

    'labels.january' => 'Gennaio',
    'labels.february' => 'Febbraio',
    'labels.march' => 'Marzo',
    'labels.april' => 'Aprile',
    'labels.may' => 'Maggio',
    'labels.june' => 'Giugno',
    'labels.july' => 'Luglio',
    'labels.august' => 'Agosto',
    'labels.september' => 'Settembre',
    'labels.october' => 'Ottobre',
    'labels.november' => 'Novembre',
    'labels.december' => 'Dicembre',

    'labels.sunday' => 'Domenica',
    'labels.monday' => 'Lunedì',
    'labels.tuesday' => 'Martedì',
    'labels.wednesday' => 'Mercoledì',
    'labels.thursday' => 'Giovedì',
    'labels.friday' => 'Venerdì',
    'labels.saturday' => 'Sabato',

    'labels.sunday_short' => 'Dom',
    'labels.monday_short' => 'Lun',
    'labels.tuesday_short' => 'Mar',
    'labels.wednesday_short' => 'Mer',
    'labels.thursday_short' => 'Gio',
    'labels.friday_short' => 'Ven',
    'labels.saturday_short' => 'Sab',


    'labels.calendar' => 'Calendario',
    'labels.location' => 'Luogo',
    'labels.description' => 'Descrizione',

    'labels.displayname' => 'Nome del calendario',
    'labels.color' => 'Colore',

    'labels.summary' => 'Titolo',
    'labels.startdate' => 'Data inizio',
    'labels.enddate' => 'Data fine',
    'labels.starttime' => 'Dalle ore',
    'labels.endtime' => 'Alle ore',
    'labels.alldayform' => 'Tutto il giorno',
    'labels.choose_date' => 'Scegli una data',

    'labels.repeat' => 'Ripeti',
    'labels.repeatno' => 'Senza ripetizioni',
    'labels.repeatdaily' => 'Quotidianamente',
    'labels.repeatweekly' => 'Settimanalmente',
    'labels.repeatmonthly' => 'Mensilmente',
    'labels.repeatyearly' => 'Annualmente',

    'labels.privacy' => 'Privacy',
    'labels.public' => 'Pubblico',
    'labels.private' => 'Privato',
    'labels.confidential' => 'Riservato',

    'labels.transp' => 'Mostra come',
    'labels.opaque' => 'Occupato',
    'labels.transparent' => 'Libero',

    'labels.generaloptions' => 'Opzioni generali',
    'labels.repeatoptions' => 'Ripetizione',
    'labels.workgroupoptions' => 'Gruppo di lavoro',
    'labels.shareoptions' => 'Condivisione',

    'labels.newcalendar' => 'Nuovo calendario',
    'labels.modifycalendar' => 'Modifica calendario',
    'labels.deletecalendar' => 'Cancella calendario',

    'labels.createevent' => 'Crea evento',
    'labels.editevent' => 'Modifica evento',
    'labels.deleteevent' => 'Cancella evento',

    'labels.calendars' => 'Calendari',
    'labels.shared_calendars' => 'Calendari condivisi',
    'labels.refresh' => 'Aggiorna',
    'labels.delete' => 'Cancella',
    'labels.add' => 'Aggiungi',
    'labels.close' => 'Chiudi',
    'labels.save' => 'Salva',
    'labels.create' => 'Crea',
    'labels.login' => 'Login',
    'labels.logout' => 'Logout',
    'labels.modify' => 'Modifica',
    'labels.cancel' => 'Annulla',
    'labels.yes' => 'Sì',
    'labels.no' => 'No',

    'labels.delete_only_this_repetition' => 'Cancella solo l\'evento corrente',
    'labels.delete_all_repetitions' => 'Cancella tutte le ricorrenze dell\'evento',

    'labels.edit_only_this_repetition' => 'Modifica solo l\'evento corrente',
    'labels.edit_all_repetitions' => 'Modifica tutte le ricorrenze dell\'evento',

    'labels.untitled' => 'Senza titolo',

    'labels.sharewith' => 'Condividi con',
    'labels.currentlysharing' => 'Calendario attualmente condiviso',
    'labels.publicurl' => 'URL per applicazioni compatibili',

    'labels.access' => 'Accesso',
    'labels.readonly' => 'Sola lettura',
    'labels.readandwrite' => 'Lettura e scrittura',

    'labels.preferences' => 'Preferenze',
    'labels.return' => 'Indietro',

    'labels.defaultcalendar' => 'Calendario predefinito',
    'labels.default_view' => 'Default view',

    'labels.toggleallcalendars' => 'Nascondi/Mostra tutti',

    'labels.minutes' => 'minuti',
    'labels.hours' => 'ore',
    'labels.days' => 'giorni',
    'labels.weeks' => 'settimane',
    'labels.months' => 'months',

    'labels.remindersoptions' => 'Promemoria',
    'labels.reminder' => 'Promemoria',

    'labels.add_reminder' => 'Add reminder',
    'labels.add_share' => 'Aggiungi condivisione',

    'labels.before_start' => 'before start',
    'labels.after' => 'After',

    'labels.start' => 'inizio',
    'labels.end' => 'fine',

    'labels.ends' => 'Fine:',
    'labels.never' => 'Never',
    'labels.occurrences' => 'ricorrenze',

    'labels.timezone' => 'Timezone',
    'labels.every' => 'Every',
    'labels.repeat_by_day' => 'Repeat on',
    'labels.repeat_by_month_day' => 'Day of month',
    'labels.repeat_explanation' => 'This event repeats',
    'labels.keep_rrule' => 'Keep original repeat rule',

    'labels.language' => 'Lingua',
    'labels.date_format' => 'Formato data',
    'labels.time_format' => 'Formato ora',
    'labels.weekstart' => 'Primo giorno della settimana',
    'labels.show_week_nb' => 'Show week numbers',
    'labels.show_now_indicator' => 'Show a marker indicating the current time',
    'labels.list_days' => 'List view days',

    'labels.view_month' => 'Month',
    'labels.view_week' => 'Week',
    'labels.view_day' => 'Day',
    'labels.view_list' => 'List',


// Messages
    'messages.error_auth' => 'Nome utente o password non validi',
    'messages.error_denied' => 'Il server ha rifiutato la richiesta (autorizzazione negata)',

    'messages.info_confirmcaldelete' => 'Vuoi davvero cancellare questo calendario?',
    'messages.info_edit_recurrent_event' => 'L\'evento si ripete. Vuoi modificare solo questo evento o anche le ricorrenze?',
    'messages.info_base_event_with_exceptions_modification' => 'Questo evento ricorrente prevede alcune eccezioni. Se usi il pulsante "modifica tutte le ricorrenze dell\'evento" cancellerai tutte le eccezioni',
    'messages.info_delete_recurrent_event' => 'Questo evento si ripete. Vuoi eliminare solo questa ricorrenza o l\'intero evento.',
    'messages.info_delete_recurrent_event_first_instance' => 'Questo evento si ripete. Eliminandolo cancellerai anche tutte le ricorrenze. Vuoi davvero farlo?',
    'messages.info_permanentremoval' => 'I dati verranno cancellati in modo definitivo',
    'messages.info_sharedby' => 'Puoi accedere a questo calendario perché %user lo ha condiviso con te',
    'messages.info_shareexplanation' => 'Puoi condividere questo calendario con altri utenti che potranno vederlo e modificarlo. Inserisci qui i nomi degli utenti separati da virgole o spazi. ',
    'messages.info_notshared' => 'Il calendario non è condiviso',
    'messages.info_noreminders' => 'Non sono stati configurati promemoria per questo evento',
    'messages.error_sessexpired' => 'La sessione è scaduta',
    'messages.error_loginagain' => 'Esegui nuovamente la login',

    'messages.error_modfailed' => 'Modifica fallita',
    'messages.error_loadevents' => 'Caricando il calendario %cal si è verificato un errore',
    'messages.error_sessrefresh' => 'Aggiornando la sessione si è verificato un errore',
    'messages.error_invalidinput' => 'Valore non valido',
    'messages.error_caldelete' => 'Errore nella cancellazione del calendario',

    'messages.error_oops' => 'Oops. Errore inatteso',
    'messages.error_interfacefailure' => 'Errore dell\'interfaccia',
    'messages.error_current_event_not_loaded' => 'L\'evento corrente non è disponibile',

    'messages.error_event_not_deleted' => 'Errore nella cancellazione dell\'evento',
    'messages.error_loading_calendar_list' => 'Errore nella lettura della lista dei calendari',
    'messages.notice_no_calendars' => 'Non ci sono calendari disponibili',
    'messages.info_rrule_not_reproducible' => 'This event has recurrence rules associated that cannot be reproduced using this interface. You can keep it as is.',
    'messages.info_rrule_protected' => 'Le impostazioni di ripetizione di questo evento sono protette.',
    'messages.error_calendarnotfound' => 'Calendario %calendar non valido',
    'messages.error_element_not_found' => 'Element not found',
    'messages.error_element_changed' => 'Element was modified while you were editing it. Please, refresh.',
    'messages.error_unexpectedhttpcode' => 'Il server ha risposto inaspettatamente con il codice %code%',
    'messages.error_network_issues' => 'Ci sono problemi di rete. Riprovare più tardi.',
    'messages.error_shareunknownusers' => 'Alcuni degli utenti specificati non esistono',
    'messages.error_empty_fields' => 'Some required fields are empty',

    'messages.help_defaultcalendar' => 'I nuovi eventi verranno automaticamente memorizzati in questo calendario. Inserire qui il calendario più utilizzato.',
    'messages.help_default_view' => 'Default view when you access your calendar',
    'messages.help_timezone' => 'Choose your current timezone. This will affect how you see existing events, and new events will be created using this timezone',
    'messages.help_list_days' => 'Choose how many consecutive days do you want to be shown on the "list" view',

    'messages.info_prefssaved' => 'Preferenze salvate',

    'messages.more_events' => '+ %count eventi',

    'messages.info_reminders_agendav_support' => 'Nota: Le notifiche vengono mostrate solo dai client CalDAV compatibili. AgenDAV non è in grado di mostrarle',

    'messages.request_error' => 'Errore nella richiesta inviata',
    'messages.page_not_found' => 'Pagina non trovata',
    'messages.internal_server_error' => 'Errore interno',

    'rrule.every' => 'Ogni',
    'rrule.until' => 'fino a',
    'rrule.day' => 'giorno',
    'rrule.days' => 'giorni',
    'rrule.week' => 'settimana',
    'rrule.weeks' => 'settimane',
    'rrule.month' => 'mese',
    'rrule.months' => 'mesi',
    'rrule.year' => 'anno',
    'rrule.years' => 'anni',
    'rrule.for' => 'per',
    'rrule.on' => 'il',
    'rrule.time' => 'volta',
    'rrule.times' => 'volte',
    'rrule.weekday' => 'giorno della settimana',
    'rrule.weekdays' => 'giorni della settimana',
    'rrule.in' => 'in',
    'rrule.on the' => 'il',
    'rrule.and' => 'e',
    'rrule.or' => 'o',
    'rrule.the' => 'il',
    'rrule.last' => 'scorso',

    'rrule.st' => '°',
    'rrule.nd' => '°',
    'rrule.rd' => '°',
    'rrule.th' => '°',
);

return $translations;
