/*
 ...
 Date: Tue Dec 13 13:06:20 2011 +0100
...
n.each(this.split(","),function(ra,ta){if(ta=="title"){ga.append("<span class='fc-header-title'><h2>&nbsp;</h2></span>");ha&&ha.addClass(q+"-corner-right");ha=null}else{var ia;if(a[ta])ia=a[ta];else if(Ja[ta])ia=function(){oa.removeClass(q+"-state-hover");a.changeView(ta)};if(ia){ra=b.theme?jb(b.buttonIcons,ta):null;var ea=jb(b.buttonText,ta),oa=n("<span class='fc-button fc-button-"+ta+" "+q+"-state-default'><span class='fc-button-inner'><span class='fc-button-content'>"+(ra?"<span class='fc-icon-wrap'><span class='ui-icon ui-icon-"+
ra+"'/></span>":ea)+"</span><span class='fc-button-effect'><span></span></span></span></span>");if(oa){oa.click(function(){oa.hasClass(q+"-state-disabled")||ia()}).mousedown(function(){oa.not("."+q+"-state-active").not("."+q+"-state-disabled").addClass(q+"-state-down")}).mouseup(function(){oa.removeClass(q+"-state-down")}).hover(function(){oa.not("."+q+"-state-active").not("."+q+"-state-disabled").addClass(q+"-state-hover")},function(){oa.removeClass(q+"-state-hover").removeClass(q+"-state-down")}).appendTo(ga);
ha||oa.addClass(q+"-corner-left");ha=oa}}}});ha&&ha.addClass(q+"-corner-right")});return ga}function g(u){P.find("h2").html(u)}function m(u){P.find("span.fc-button-"+u).addClass(q+"-state-active")}function j(u){P.find("span.fc-button-"+u).removeClass(q+"-state-active")}function t(u){P.find("span.fc-button-"+u).addClass(q+"-state-disabled")}function A(u){P.find("span.fc-button-"+u).removeClass(q+"-state-disabled")}var R=this;R.render=e;R.destroy=c;R.updateTitle=g;R.activateButton=m;R.deactivateButton=
j;R.disableButton=t;R.enableButton=A;var P=n([]),q}function $b(a,b){function e(d,H){return!da||d<da||H>la}function c(d,H){da=d;la=H;sa=d.getTimezoneOffset()*60*1E3;I=H.getTimezoneOffset()*60*1E3;T=[];d=++o;V=H=S.length;for(var x=0;x<H;x++)f(S[x],d)}function f(d,H){g(d,function(x){if(H==o){if(x){for(var $=0;$<x.length;$++){x[$].source=d;pa(x[$])}T=T.concat(x)}V--;V||va(T)}})}function g(d,H){var x,$=Aa.sourceFetchers,C;for(x=0;x<$.length;x++){C=$[x](d,da,la,H);if(C===true)return;else if(typeof C=="object"){g(C,
...
this,arguments);H()},complete:function(){$a(O,this,arguments);ga()}}))}else H()}function m(d){if(d=j(d)){V++;f(d,o)}}function j(d){if(n.isFunction(d)||n.isArray(d))d={events:d};else if(typeof d=="string")d={url:d};if(typeof d=="object"){ha(d);S.push(d);return d}}function t(d){S=n.grep(S,function(H){return!ra(H,d)});T=n.grep(T,function(H){return!ra(H.source,d)});va(T)}function A(d){var H,x=T.length,$,C=oa().defaultEventEnd,i=d.start-d._start,F=d.end?d.end-(d._end||C(d)):0;for(H=0;H<x;H++){$=T[H];if($._id==
d._id&&$!=d){$.start=new Date(+$.start+i);$.end=d.end?$.end?new Date(+$.end+F):new Date(+C($)+F):null;$.title=d.title;$.url=d.url;$.allDay=d.allDay;$.className=d.className;$.editable=d.editable;$.color=d.color;$.backgroudColor=d.backgroudColor;$.borderColor=d.borderColor;$.textColor=d.textColor;pa($)}}pa(d);va(T)}function R(d,H){pa(d);if(!d.source){if(H){qa.events.push(d);d.source=qa}T.push(d)}va(T)}function P(d){if(d){if(!n.isFunction(d)){var H=d+"";d=function($){return $._id==H}}T=n.grep(T,d,true);
for(x=0;x<S.length;x++)if(n.isArray(S[x].events))S[x].events=n.grep(S[x].events,d,true)}else{T=[];for(var x=0;x<S.length;x++)if(n.isArray(S[x].events))S[x].events=[]}va(T)}function q(d){if(n.isFunction(d))return n.grep(T,d);else if(d){d+="";return n.grep(T,function(H){return H._id==d})}return T}function u(){Y++||ea("loading",null,true)}function ga(){--Y||ea("loading",null,false)}function pa(d){var H=d.source||{},x=Ta(H.ignoreTimezone,a.ignoreTimezone);d._id=d._id||(d.id===na?"_fc"+bc++:d.id+"");if(d.date){if(!d.start)d.start=
d.date;delete d.date}d._start=M(d.start=kb(d.start,x));d.end=kb(d.end,x);if(d.end&&d.end<=d.start)d.end=null;d._end=d.end?M(d.end):null;if(d.allDay===na)d.allDay=Ta(H.allDayDefault,a.allDayDefault);if(d.className){if(typeof d.className=="string")d.className=d.className.split(/\s+/)}else d.className=[]}function ha(d){if(d.className){if(typeof d.className=="string")d.className=d.className.split(/\s+/)}else d.className=[];for(var H=Aa.sourceNormalizers,x=0;x<H.length;x++)H[x](d)}function ra(d,H){return d&&
...
"marginTop",true))||0)+(parseFloat(n.curCSS(a[0],"marginBottom",true))||0)}function kc(a){return(parseFloat(n.curCSS(a[0],"borderTopWidth",true))||0)+(parseFloat(n.curCSS(a[0],"borderBottomWidth",true))||0)}function Za(a,b){b=typeof b=="number"?b+"px":b;a.each(function(e,c){c.style.cssText+=";min-height:"+b+";_height:"+b})}function xb(){}function Gb(a,b){return a-b}function Hb(a){return Math.max.apply(Math,a)}function Pa(a){return(a<10?"0":"")+a}function jb(a,b){if(a[b]!==na)return a[b];b=b.split(/(?=[A-Z])/);
for(var e=b.length-1,c;e>=0;e--){c=a[b[e].toLowerCase()];if(c!==na)return c}return a[""]}function Qa(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/'/g,"&#039;").replace(/"/g,"&quot;").replace(/\n/g,"<br />")}function Ib(a){return a.id+"/"+a.className+"/"+a.style.cssText.replace(/(^|;)\s*(top|left|width|height)\s*:[^;]*/ig,"")}function qb(a){a.attr("unselectable","on").css("MozUserSelect","none").bind("selectstart.ui",function(){return false})}function ab(a){a.children().removeClass("fc-first fc-last").filter(":first-child").addClass("fc-first").end().filter(":last-child").addClass("fc-last")}
...
[a];if(a){var c,f;for(c=0;c<a.length;c++)f=a[c].apply(b,e)||f;return f}}function Ta(){for(var a=0;a<arguments.length;a++)if(arguments[a]!==na)return arguments[a]}function mc(a,b){function e(j,t){if(t){hb(j,t);j.setDate(1)}j=M(j,true);j.setDate(1);t=hb(M(j),1);var A=M(j),R=M(t),P=f("firstDay"),q=f("weekends")?0:1;if(q){Fa(A);Fa(R,-1,true)}ca(A,-((A.getDay()-Math.max(P,q)+7)%7));ca(R,(7-R.getDay()+Math.max(P,q))%7);P=Math.round((R-A)/(Ab*7));if(f("weekMode")=="fixed"){ca(R,(6-P)*7);P=6}c.title=m(j,
...
Ca(ba,Q);ba=Ca(wa,Q)}A(q(fa,xa,fa,ba-1))}ca(Q,7);ca(U,7)}}function q(w,J,Q,U){w=r.rect(w,J,Q,U,a);return d(w,a)}function u(w){return M(w)}function ga(w,J){P(w,ca(M(J),1),true)}function pa(){H()}function ha(w,J,Q){var U=va(w);Y("dayClick",B[U.row*G+U.col],w,J,Q)}function ra(w,J){K.start(function(Q){H();Q&&q(Q.row,Q.col,Q.row,Q.col)},J)}function ta(w,J,Q){var U=K.stop();H();if(U){U=qa(U);Y("drop",w,U,true,J,Q)}}function ia(w){return M(w.start)}function ea(w){return L.left(w)}function oa(w){return L.right(w)}
function va(w){return{row:Math.floor(Ca(w,o.visStart)/7),col:la(w.getDay())}}function qa(w){return S(w.row,w.col)}function S(w,J){return ca(M(o.visStart),w*7+J*E+Z)}function da(w){return S(Math.floor(w/G),w%G)}function la(w){return(w-Math.max(ka,ja)+G)%G*E+Z}function sa(w){return O.eq(w)}function I(){return{left:0,right:W}}var o=this;o.renderBasic=c;o.setHeight=j;o.setWidth=t;o.renderDayOverlay=P;o.defaultSelectionEnd=u;o.renderSelection=ga;o.clearSelection=pa;o.reportDayClick=ha;o.dragStart=ra;o.dragStop=
ta;o.defaultEventEnd=ia;o.getHoverListener=function(){return K};o.colContentLeft=ea;o.colContentRight=oa;o.dayOfWeekCol=la;o.dateCell=va;o.cellDate=qa;o.cellIsAllDay=function(){return true};o.allDayRow=sa;o.allDayBounds=I;o.getRowCnt=function(){return v};o.getColCnt=function(){return G};o.getColWidth=function(){return s};o.getDaySegmentContainer=function(){return X};Kb.call(o,a,b,e);Lb.call(o);Mb.call(o);pc.call(o);var V=o.opt,Y=o.trigger,T=o.clearEvents,d=o.renderOverlay,H=o.clearOverlays,x=o.daySelectionMousedown,
...
h)}else F("dayClick",L[N],y,true,h)}}function pa(h,N,y){y&&Na.build();var D=M(C.visStart);if(Ub){y=Ca(N,D)*Ha+Ia+1;h=Ca(h,D)*Ha+Ia+1}else{y=Ca(h,D);h=Ca(N,D)}y=Math.max(0,y);h=Math.min(Ba,h);y<h&&q(ha(0,y,0,h-1))}function ha(h,N,y,D){h=Na.rect(h,N,y,D,ka);return B(h,ka)}function ra(h,N){for(var y=M(C.visStart),D=ca(M(y),1),ua=0;ua<Ba;ua++){var za=new Date(Math.max(y,h)),Da=new Date(Math.min(D,N));if(za<Da){var Ea=ua*Ha+Ia;Ea=Na.rect(0,Ea,0,Ea,J);za=da(y,za);Da=da(y,Da);Ea.top=za;Ea.height=Da-za;u(B(Ea,
J))}ca(y,1);ca(D,1)}}function ta(h){return cb.left(h)}function ia(h){return cb.right(h)}function ea(h){return{row:Math.floor(Ca(h,C.visStart)/7),col:S(h.getDay())}}function oa(h){var N=va(h.col);h=h.row;i("allDaySlot")&&h--;h>=0&&ya(N,La+h*i("slotMinutes"));return N}function va(h){return ca(M(C.visStart),h*Ha+Ia)}function qa(h){return i("allDaySlot")&&!h.row}function S(h){return(h-Math.max(Tb,Sb)+Ba)%Ba*Ha+Ia}function da(h,N){h=M(h,true);if(N<ya(M(h),La))return 0;if(N>=ya(M(h),bb))return U.height();
h=i("slotMinutes");N=N.getHours()*60+N.getMinutes()-La;var y=Math.floor(N/h),D=ub[y];if(D===na)D=ub[y]=U.find("tr:eq("+y+") td div")[0].offsetTop;return Math.max(0,Math.round(D-1+Xa*(N%h/h)))}function la(){return{left:Ma,right:Ga-vb}}function sa(){return aa}function I(h){var N=M(h.start);if(h.allDay)return N;return ya(N,i("defaultEventMinutes"))}function o(h,N){if(N)return M(h);return ya(M(h),i("slotMinutes"))}function V(h,N,y){if(y)i("allDaySlot")&&pa(h,ca(M(N),1),true);else Y(h,N)}function Y(h,
...
N)}function T(){z();if(xa){xa.remove();xa=null}}function d(h){if(h.which==1&&i("selectable")){X(h);var N;Ra.start(function(y,D){T();if(y&&y.col==D.col&&!qa(y)){D=oa(D);y=oa(y);N=[D,ya(M(D),i("slotMinutes")),y,ya(M(y),i("slotMinutes"))].sort(Gb);Y(N[0],N[3])}else N=null},h);n(document).one("mouseup",function(y){Ra.stop();if(N){+N[0]==+N[1]&&H(N[0],false,y);l(N[0],N[3],false,y)}})}}function H(h,N,y){F("dayClick",L[S(h.getDay())],h,N,y)}function x(h,N){Ra.start(function(y){z();if(y)if(qa(y))ha(y.row,
y.col,y.row,y.col);else{y=oa(y);var D=ya(M(y),i("defaultEventMinutes"));ra(y,D)}},N)}function $(h,N,y){var D=Ra.stop();z();D&&F("drop",h,oa(D),qa(D),N,y)}var C=this;C.renderAgenda=c;C.setWidth=t;C.setHeight=j;C.beforeHide=R;C.afterShow=P;C.defaultEventEnd=I;C.timePosition=da;C.dayOfWeekCol=S;C.dateCell=ea;C.cellDate=oa;C.cellIsAllDay=qa;C.allDayRow=sa;C.allDayBounds=la;C.getHoverListener=function(){return Ra};C.colContentLeft=ta;C.colContentRight=ia;C.getDaySegmentContainer=function(){return ja};
C.getSlotSegmentContainer=function(){return Q};C.getMinMinute=function(){return La};C.getMaxMinute=function(){return bb};C.getBodyContent=function(){return J};C.getRowCnt=function(){return 1};C.getColCnt=function(){return Ba};C.getColWidth=function(){return db};C.getSlotHeight=function(){return Xa};C.defaultSelectionEnd=o;C.renderDayOverlay=pa;C.renderSelection=V;C.clearSelection=T;C.reportDayClick=H;C.dragStart=x;C.dragStop=$;Kb.call(C,a,b,e);Lb.call(C);Mb.call(C);sc.call(C);var i=C.opt,F=C.trigger,
...
aa;ua=D.offset().top;h[0]=[ua,ua+D.outerHeight()]}for(var Da=J.offset().top,Ea=w.offset().top,tc=Ea+w.outerHeight(),fb=0;fb<tb;fb++)h.push([y(Da+Xa*fb),y(Da+Xa*(fb+1))])});Ra=new Ob(Na);cb=new Pb(function(h){return k.eq(h)})}function sc(){function a(p,s){ta(p);var v,G=p.length,r=[],K=[];for(v=0;v<G;v++)p[v].allDay?r.push(p[v]):K.push(p[v]);if(u("allDaySlot")){V(e(r),s);oa()}g(c(K),s)}function b(){ia();va().empty();qa().empty()}function e(p){p=ob(nb(p,n.map(p,Ua),q.visStart,q.visEnd));var s,v=p.length,
...
r.outerHeight-r.vsides);aa[0].style.height=Q+"px";K=r.event;if(r.contentTop!==na&&Q-r.contentTop<10){aa.find("div.fc-event-time").text(X(K.start,u("timeFormat"))+" - "+K.title);aa.find("div.fc-event-title").remove()}ga("eventAfterRender",K,K,aa)}}}function m(p,s){var v="<",G=p.url,r=Jb(p,u),K=r?" style='"+r+"'":"",L=["fc-event","fc-event-skin","fc-event-vert"];pa(p)&&L.push("fc-event-draggable");s.isStart&&L.push("fc-corner-top");s.isEnd&&L.push("fc-corner-bottom");L=L.concat(p.className);if(p.source)L=
...
return I}function f(I,o){return b.trigger.apply(b,[I,o||ea].concat(Array.prototype.slice.call(arguments,2),[ea]))}function g(I){return j(I)&&!c("disableDragging")}function m(I){return j(I)&&!c("disableResizing")}function j(I){return Ta(I.editable,(I.source||{}).editable,c("editable"))}function t(I){S={};var o,V=I.length,Y;for(o=0;o<V;o++){Y=I[o];if(S[Y._id])S[Y._id].push(Y);else S[Y._id]=[Y]}}function A(I){return I.end?M(I.end):oa(I)}function R(I,o){da.push(o);if(la[I._id])la[I._id].push(o);else la[I._id]=
[o]}function P(){da=[];la={}}function q(I,o){o.click(function(V){if(!o.hasClass("ui-draggable-dragging")&&!o.hasClass("ui-resizable-resizing"))return f("eventClick",this,I,V)}).hover(function(V){f("eventMouseover",this,I,V)},function(V){f("eventMouseout",this,I,V)})}function u(I,o){pa(I,o,"show")}function ga(I,o){pa(I,o,"hide")}function pa(I,o,V){I=la[I._id];var Y,T=I.length;for(Y=0;Y<T;Y++)if(!o||I[Y][0]!=o[0])I[Y][V]()}function ha(I,o,V,Y,T,d,H){var x=o.allDay,$=o._id;ta(S[$],V,Y,T);f("eventDrop",
I,o,V,Y,T,function(){ta(S[$],-V,-Y,x);qa($)},d,H);qa($)}function ra(I,o,V,Y,T,d){var H=o._id;ia(S[H],V,Y);f("eventResize",I,o,V,Y,function(){ia(S[H],-V,-Y);qa(H)},T,d);qa(H)}function ta(I,o,V,Y){V=V||0;for(var T,d=I.length,H=0;H<d;H++){T=I[H];if(Y!==na)T.allDay=Y;ya(ca(T.start,o,true),V);if(T.end)T.end=ya(ca(T.end,o,true),V);va(T,sa)}}function ia(I,o,V){V=V||0;for(var Y,T=I.length,d=0;d<T;d++){Y=I[d];Y.end=ya(ca(A(Y),o,true),V);va(Y,sa)}}var ea=this;ea.element=a;ea.calendar=b;ea.name=e;ea.opt=c;ea.trigger=
f;ea.isEventDraggable=g;ea.isEventResizable=m;ea.reportEvents=t;ea.eventEnd=A;ea.reportEventElement=R;ea.reportEventClear=P;ea.eventElementHandlers=q;ea.showEvents=u;ea.hideEvents=ga;ea.eventDrop=ha;ea.eventResize=ra;var oa=ea.defaultEventEnd,va=b.normalizeEvent,qa=b.reportEventChange,S={},da=[],la={},sa=b.options}function Qb(){function a(i,F){var O=T(),B=qa(),z=S(),l=0,X,W,p=i.length,s,v;O[0].innerHTML=e(i);c(i,O.children());f(i);g(i,O,F);m(i);j(i);t(i);F=A();for(O=0;O<B;O++){X=[];for(W=0;W<z;W++)X[W]=
...
B}}Db(F,i,d)}function m(i){var F,O=i.length,B,z,l,X,W={};for(F=0;F<O;F++){B=i[F];if(z=B.element){l=B.key=Ib(z[0]);X=W[l];if(X===na)X=W[l]=pb(z,true);B.hsides=X}}}function j(i){var F,O=i.length,B,z;for(F=0;F<O;F++){B=i[F];if(z=B.element)z[0].style.width=Math.max(0,B.outerWidth-B.hsides)+"px"}}function t(i){var F,O=i.length,B,z,l,X,W={};for(F=0;F<O;F++){B=i[F];if(z=B.element){l=B.key;X=W[l];if(X===na)X=W[l]=Fb(z);B.outerHeight=z[0].offsetHeight+X}}}function A(){var i,F=qa(),O=[];for(i=0;i<F;i++)O[i]=
da(i).find("td:first div.fc-day-content > div");return O}function R(i){var F,O=i.length,B=[];for(F=0;F<O;F++)B[F]=i[F][0].offsetTop;return B}function P(i,F){var O,B=i.length,z,l;for(O=0;O<B;O++){z=i[O];if(l=z.element){l[0].style.top=F[z.row]+(z.top||0)+"px";z=z.event;pa("eventAfterRender",z,z,l)}}}function q(i,F,O){var B=ga("isRTL"),z=B?"w":"e",l=F.find("div.ui-resizable-"+z),X=false;qb(F);F.mousedown(function(W){W.preventDefault()}).click(function(W){if(X){W.preventDefault();W.stopImmediatePropagation()}});
l.mousedown(function(W){function p(ja){pa("eventResizeStop",this,i,ja);n("body").css("cursor","");s.stop();$();k&&va(this,i,k,0,ja);setTimeout(function(){X=false},0)}if(W.which==1){X=true;var s=u.getHoverListener(),v=qa(),G=S(),r=B?-1:1,K=B?G-1:0,L=F.css("top"),k,E,Z=n.extend({},i),ka=V(i.start);C();n("body").css("cursor",z+"-resize").one("mouseup",p);pa("eventResizeStart",this,i,W);s.start(function(ja,ma){if(ja){var aa=Math.max(ka.row,ja.row);ja=ja.col;if(v==1)aa=0;if(aa==ka.row)ja=B?Math.min(ka.col,
ja):Math.max(ka.col,ja);k=aa*7+ja*r+K-(ma.row*7+ma.col*r+K);ma=ca(ta(i),k,true);if(k){Z.end=ma;aa=E;E=b(Y([Z]),O.row,L);E.find("*").css("cursor",z+"-resize");aa&&aa.remove();oa(i)}else if(E){ea(i);E.remove();E=null}$();x(i.start,ca(M(ma),1))}},W)}})}var u=this;u.renderDaySegs=a;u.resizableDayEvent=q;var ga=u.opt,pa=u.trigger,ha=u.isEventDraggable,ra=u.isEventResizable,ta=u.eventEnd,ia=u.reportEventElement,ea=u.showEvents,oa=u.hideEvents,va=u.eventResize,qa=u.getRowCnt,S=u.getColCnt,da=u.allDayRow,
...
t,A=-1,R=-1;for(t=0;t<m;t++)if(g>=e[t][0]&&g<e[t][1]){A=t;break}for(t=0;t<j;t++)if(f>=c[t][0]&&f<c[t][1]){R=t;break}return A>=0&&R>=0?{row:A,col:R}:null};b.rect=function(f,g,m,j,t){t=t.offset();return{top:e[f][0]-t.top,left:c[g][0]-t.left,width:c[j][1]-c[g][0],height:e[m][1]-e[f][0]}}}function Ob(a){function b(j){xc(j);j=a.cell(j.pageX,j.pageY);if(!j!=!m||j&&(j.row!=m.row||j.col!=m.col)){if(j){g||(g=j);f(j,g,j.row-g.row,j.col-g.col)}else f(j,g);m=j}}var e=this,c,f,g,m;e.start=function(j,t,A){f=j;
g=m=null;a.build();b(t);c=A||"mousemove";n(document).bind(c,b)};e.stop=function(){n(document).unbind(c,b);return m}}function xc(a){if(a.pageX===na){a.pageX=a.originalEvent.pageX;a.pageY=a.originalEvent.pageY}}function Pb(a){function b(m){return c[m]=c[m]||a(m)}var e=this,c={},f={},g={};e.left=function(m){return f[m]=f[m]===na?b(m).position().left:f[m]};e.right=function(m){return g[m]=g[m]===na?e.left(m)+b(m).width():g[m]};e.clear=function(){c={};f={};g={}}}var Ya={defaultView:"month",aspectRatio:1.35,
...
{header:{left:"next,prev today",center:"",right:"title"},buttonText:{prev:"&nbsp;&#9658;&nbsp;",next:"&nbsp;&#9668;&nbsp;",prevYear:"&nbsp;&gt;&gt;&nbsp;",nextYear:"&nbsp;&lt;&lt;&nbsp;"},buttonIcons:{prev:"circle-triangle-e",next:"circle-triangle-w"}},Aa=n.fullCalendar={version:"1.5.2"},Ja=Aa.views={};n.fn.fullCalendar=function(a){if(typeof a=="string"){var b=Array.prototype.slice.call(arguments,1),e;this.each(function(){var f=n.data(this,"fullCalendar");if(f&&n.isFunction(f[a])){f=f[a].apply(f,
b);if(e===na)e=f;a=="destroy"&&n.removeData(this,"fullCalendar")}});if(e!==na)return e;return this}var c=a.eventSources||[];delete a.eventSources;if(a.events){c.push(a.events);delete a.events}a=n.extend(true,{},Ya,a.isRTL||a.isRTL===na&&Ya.isRTL?yc:{},a);this.each(function(f,g){f=n(g);g=new Yb(f,a,c);f.data("fullCalendar",g);g.render()});return this};Aa.sourceNormalizers=[];Aa.sourceFetchers=[];var ac={dataType:"json",cache:false},bc=1;Aa.addDays=ca;Aa.cloneDate=M;Aa.parseDate=kb;Aa.parseISO8601=
